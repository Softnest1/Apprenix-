      ...
      return [
        {
          ...
          items: [
            { path: '/aide-devoirs',        label: 'Aide aux devoirs',       icon: Brain,      demo: true },
      ...
      },
      {
        ...
        items: [
          ...
          ...(!isPrimaire ? [
            ...
            { path: '/chansons-educatives', label: 'Chansons éducatives', icon: Bell  },
          ] : []),
        ],
      },
      {
...
const ALL_PAGE_LABELS: { path: string; label: string; exact?: boolean }[] = [
  ...
  { path: '/aide-devoirs',               label: 'Aide aux devoirs'           },
...
const NAVBAR_GROUPS: NavGroup[] = [
  // 1 ── Découvrir les outils — tous en démo interactive, 2 colonnes
  {
    ...
    cols: 2,
    items: [
      { label: 'Aide aux devoirs',       path: '/aide-devoirs',        icon: Brain,      desc: '✨ Démo — Fiches méthode & exercices guidés',   badge: 'Bêta' },
...
// La sidebar (EtudiantLayout) couvre déjà tous les outils (/aide-devoirs, /scanner,
...
const NAVBAR_GROUPS_TEACHER: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Tester les outils',  path: '/aide-devoirs',     icon: Brain,           desc: 'Tester les outils élèves'               },
...
const Breadcrumb: React.FC = () => {
  ...
  if (segments.length === 0) return null; // pas de breadcrumb sur l'accueil
        ...
        {crumbs.map((crumb, i) => {
          ...
          return (
            <li key={crumb.href} className="flex items-center gap-1 min-w-0">
              {i > 0 && (
                <ChevronRight className="w-3.5 h-3.5 shrink-0 text-muted-foreground/40 mx-0.5" aria-hidden="true" />
              )}
              {isLast ? (
                <span
                  aria-current="page"
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-primary/8 text-primary border border-primary/20 truncate max-w-[200px]"
                >
                  {crumb.label}
                </span>
              ) : (
                <button
                  ...
                  className={cn(
                    'inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-150 truncate max-w-[160px]',
                    i === 0
                      ? 'bg-muted text-muted-foreground border border-border hover:bg-primary/8 hover:text-primary hover:border-primary/20'
...
const MobileBottomBar: React.FC = () => {
  ...
  const tabs = isAuthenticated
    ? [
        ...
        { label: 'Outils',     icon: Zap,            path: '/aide-devoirs',       exact: false },
    ...
    : [
        { label: 'Accueil',   icon: Home,    path: '/',              exact: true  },
        { label: 'Démos',     icon: Zap,     path: '/aide-devoirs',       exact: false },
        { label: 'Réponses',  icon: Sparkles,path: '/base-reponses', exact: false },
