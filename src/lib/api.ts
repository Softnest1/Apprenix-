...
/** Récupère les préférences d'accessibilité d'un utilisateur depuis Supabase. */
export async function getAccessibilityPrefs(userId: string): Promise<AccessibilityPrefs | null> {
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error || !data) return null;
  const raw = (data as Record<string, unknown>).accessibility_prefs;
  if (!raw || typeof raw !== 'object') return null;
  return raw as AccessibilityPrefs;
}
...
export async function updateAccessibilityPrefs(
  ...
  if (!userId || userId === 'local') throw new Error('userId invalide');
